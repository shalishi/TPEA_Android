...
public class JPTabBar extends LinearLayout implements ViewPager.OnPageChangeListener {
    ...
    //默认的没有选中的文字和图标的颜色
    private static final int DEFAULT_NORMAL_COLOR = 0xff666666;
    //默认的上下背景间隔
    private static final int DEFAULT_MARGIN = 8;
    //默认的选中颜色
    private static final int DEFAULT_SELECT_COLOR = 0xffE45D30;
    //默认是否接受颜色随着字体变化
    ...
    private OnClickAddProductListener onClickAddProductListener;
    ...
    private void initFromAttribute() {
        ...
        tabBarType = mAttribute.getInt(R.styleable.JPTabBar_TabBarType, 1);
        ...
        if (!isInEditMode()) {
            ...
            for (int i = 0; i < mJPTabItems.length; i++) {
                final int temp = i;
                mJPTabItems[i] = new JPTabItem.Builder(mContext).setTitle(mTitles == null ? null : mTitles[i]).setIndex(temp)
    ...
    public void setOnClickAddProductListener(OnClickAddProductListener onClickAddProductListener) {
        this.onClickAddProductListener = onClickAddProductListener;
    }

    public interface OnClickAddProductListener {
